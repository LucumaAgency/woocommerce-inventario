...
class MSP_Pruebas {
	...
	private function escenario_venta( $sede_id, $atascada, $dias_atras ) {
		...
		if ( ! MSP_Cola::activa() ) {
			return __( 'Enciende la emisión automática en Facturación: sin ella no se genera ningún comprobante.', 'multisede-pos' );
		}

		$producto_id = $this->producto_con_stock( $sede_id );
		if ( ! $producto_id ) {
			return __( 'Esa tienda no tiene ningún producto con stock disponible.', 'multisede-pos' );
		}

		$producto = wc_get_product( $producto_id );
		if ( ! $producto ) {
			return __( 'El producto con stock ya no existe.', 'multisede-pos' );
		}
		...
		if ( ! MSP_Stock::descontar_si_hay( $producto_id, $sede_id, 1 ) ) {
			return __( 'No se pudo descontar el stock.', 'multisede-pos' );
		}
		...
		if ( ! $comprobante ) {
			...
			return __( 'La venta se creó pero no generó comprobante. Revisa que la sede tenga serie de boleta.', 'multisede-pos' );
		}
		...
		$actual = MSP_Comprobante::obtener( (int) $comprobante['id'] );
